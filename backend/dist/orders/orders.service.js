...
let OrdersService = class OrdersService {
    ...
    async create(userId, dto) {
                ...
                const order = await this.prisma.$transaction(async (tx) => {
                    ...
                    for (const item of cart.cart_items) {
                        ...
                        if (variant.stock_quantity < item.quantity) {
                            throw new common_1.BadRequestException(`insufficient stock: only ${variant.stock_quantity} of ${variant.sku} available`);
                        }
                    ...
                    for (const item of cart.cart_items) {
                        ...
                        if (result.count === 0) {
                            throw new common_1.BadRequestException(`insufficient stock: only ${item.product_variants.stock_quantity} of ${item.product_variants.sku} available`);
                        }
                    }
        ...
        throw new common_1.BadRequestException('could not place order, please retry');
    }
    ...
    async resolveAddress(userId, addressId) {
        if (addressId) {
        ...
        if (!address) {
            throw new common_1.BadRequestException('no shipping address on file');
        }
        return address;
    }
    generateOrderNumber() {
