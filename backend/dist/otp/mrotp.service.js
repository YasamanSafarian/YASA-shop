...
let MrotpService = MrotpService_1 = class MrotpService {
    configService;
    logger = new common_1.Logger(MrotpService_1.name);
    ...
    async sendOtp(mobile, validTimeMinutes = 3) {
        ...
        return this.request('setRandomOTP', body, (data) => {
            ...
            if (!ok) {
                this.logger.warn(`setRandomOTP failed: code=${data.code} message=${data.message ?? ''}`);
            }
            return {
                ok,
                code: data.code,
...
exports.MrotpService = MrotpService = MrotpService_1 = __decorate([
